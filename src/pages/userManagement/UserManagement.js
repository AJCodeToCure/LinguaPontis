                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {agenciesData.map((agency) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-3">{agency.group}</td>
                                            ...
                                            <td className="p-3">
                                                <button
                                                    className={`px-4 py-1 rounded-[8px] text-xs ${agency.is_disable || deactivatedAgencies.includes(agency.user_id)
                                                        ? 'bg-gray-300 text-gray-700 cursor-not-allowed'  // Disabled button style
                                                        : 'bg-yellow-200 text-yellow-800 border border-yellow-800'
                                                        }`}
                                                    onClick={() => handleDeactivate(agency.user_id)}
                                                    disabled={agency.is_disable || deactivatedAgencies.includes(agency.user_id)}  // Disable the button if already deactivated
                                                >
                                                    {agency.is_disable || deactivatedAgencies.includes(agency.user_id) ? 'Deactivated' : 'Deactivate'}
                                                </button>
                                            </td>
                                        </tr>
                                    ))}
