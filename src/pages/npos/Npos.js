...
function Npos() {
    ...
    // Filter agencies by company_name and email
    const filterAgencies = (agencies) => {
        return agencies.filter((agency) => {
            const companyNameMatch = agency.company_name.toLowerCase().includes(searchQuery.toLowerCase());
            const emailMatch = agency.email?.toLowerCase().includes(searchQuery.toLowerCase());
            return companyNameMatch || emailMatch;
        });
    };

    // Get filtered agencies based on search query
    const getFilteredAgencies = () => {
        return filterAgencies(npos);
    };

    // Pagination Logic
    const indexOfLastAgency = currentPage * itemsPerPage;
    const indexOfFirstAgency = indexOfLastAgency - itemsPerPage;
    const currentAgencies = getFilteredAgencies().slice(indexOfFirstAgency, indexOfLastAgency);
                    ...
                    <div className="container mt-20 max-sm:w-full w-10/12 mx-auto">
                            ...
                            <div className="flex items-center max-sm:flex-col">
                                ...
                                <div className="relative mr-4 max-sm:mt-5 max-sm:mb-5">
                                    ...
                                    <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400">
                                        {/* Assuming MagnifyingIcons is the search icon */}
                                    </span>
                                </div>
                        ...
                        <div className="mt-4 flex justify-between items-center">
                            <p className="text-sm font-[Poppins] text-gray-600">
                                Showing {indexOfFirstAgency + 1} to {Math.min(indexOfLastAgency, getFilteredAgencies().length)} of {getFilteredAgencies().length} entries
                            </p>
                            <div className="flex">
                                <button
                                ...
                                {[...Array(Math.ceil(getFilteredAgencies().length / itemsPerPage)).keys()].map((num) => (
                                    <button
                                        key={num + 1}
                                ...
                                <button
                                    ...
                                    onClick={() => paginate(Math.min(currentPage + 1, Math.ceil(getFilteredAgencies().length / itemsPerPage)))}
                                >
